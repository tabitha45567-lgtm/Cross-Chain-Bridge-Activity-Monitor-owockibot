...
export default function BridgeMonitor() {
  ...
  const scanOnchain = useCallback(async (address, key, bridgeList) => {
    ...
    try {
      ...
      const fromBlock = "0x" + Math.max(0, latest - LOG_BLOCK_RANGE).toString(16);
      ...
      for (const b of bridgeList) {
        try {
            ...
            rpcCall("eth_getLogs", [
              {
                ...
                fromBlock,
                toBlock: "latest",
                topics: [TRANSFER_TOPIC, null, padTopic(b.address)],
              },
            ]),
            rpcCall("eth_getLogs", [
              {
                ...
                fromBlock,
                toBlock: "latest",
                topics: [TRANSFER_TOPIC, padTopic(b.address), null],
              },
            ]),
          ]);
          ...
          for (const log of outLogs) {
            bOut += Number(BigInt(log.data)) / 10 ** decimals;
            outAddrs.add(topicToAddress(log.topics[1]));
          }
          for (const log of inLogs) {
            bIn += Number(BigInt(log.data)) / 10 ** decimals;
            inAddrs.add(topicToAddress(log.topics[2]));
          }
          outVolume += bOut;
          inVolume += bIn;
          perBridge.push({ name: b.name, in: bIn, out: bOut, color: b.color });
          anyOk = true;
        } catch (e) {
          anyError = String(e.message || e);
          perBridge.push({ name: b.name, in: 0, out: 0, color: b.color });
        }
      }
      ...
      setOnchain({
        ...
        perBridge,
        sampleNote: `Reading Transfer events directly from Base's public RPC over the last ~${LOG_BLOCK_RANGE.toLocaleString()} blocks (no explorer API, no key needed).`,
      });
    } catch (e) {
      ...
      <main className="px-6 py-6 max-w-6xl mx-auto space-y-8">
        <section className="rounded-xl border border-[#232C4D] bg-[#121933] p-5">
          ...
          <p className="text-xs text-[#8B93B8] mt-3 flex items-start gap-1.5">
            ...
            No API key needed — the on-chain scanner reads Transfer events
            directly from Base's public RPC, the same endpoint wallets use.
            Explorer APIs (Etherscan, Blockscout) kept changing their pricing
            and browser-access rules, so this skips them entirely.
          </p>
        </section>
        ...
        <section className="rounded-xl border border-[#232C4D] bg-[#121933] p-5">
          ...
          {onchain.status === "error" && (
            <div className="flex items-start gap-2 text-sm text-[#FF6B6B]">
              <AlertTriangle size={16} className="mt-0.5 shrink-0" />
              <p>{onchain.error || "Could not reach Base's public RPC."}</p>
            </div>
          )}
          {onchain.status === "ok" && (
        ...
        <footer className="text-xs text-[#4A5280] pb-6 space-y-1">
          <p>
            Sources: LI.FI aggregator API (li.quest) · Base public RPC
            (eth_getLogs, direct on-chain read) · bridge-contract registry
            defined in source.
          </p>
